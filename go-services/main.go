...
func init() {
	...
	db_connection := "user=postgres dbname=chicago_business_intelligence password=root host=/cloudsql/bilguun3:us-central1:mypostgres sslmode=disable port = 5432"
...
func GetTaxiTrips(db *sql.DB) {
	...
	geocoder.ApiKey = "AIzaSyCsJ2_5irvPKgjUg4Ppu39bdzLCXDC8Lzc"
...
func GetCovidDetails(db *sql.DB) {
	...
	fmt.Println("Completed Inserting Rows into the COVID Details Table")
}
...
func GetCCVIDetails(db *sql.DB) {
	fmt.Println("GetCCVIDetails: Collecting CCVI Details Data")

	// Define table schema for CCVI details
	...
	create_table := `CREATE TABLE IF NOT EXISTS ccvi_details (
		...
		community_area_or_zip_code VARCHAR(50),
		community_name VARCHAR(255),
		ccvi_score DOUBLE PRECISION,
		ccvi_category VARCHAR(50)
	)`
	_, err = db.Exec(create_table)
	if err != nil {
		panic(err)
	...
	fmt.Println("Created Table for CCVI Details")

	// URL to fetch CCVI details
	...
	for _, record := range ccvi_data_list {
		...
		community_area_or_zip_code := record.Community_area_or_ZIP_code
		...
		ccvi_score, _ := strconv.ParseFloat(record.CCVI_score, 64)
		ccvi_category := record.CCVI_category

		sql := `INSERT INTO ccvi_details (
			geography_type, community_area_or_zip_code, community_name, ccvi_score, ccvi_category
		) VALUES ($1, $2, $3, $4, $5)`

		_, err := db.Exec(sql, geography_type, community_area_or_zip_code, community_name, ccvi_score, ccvi_category)

		if err != nil {
			fmt.Printf("Error inserting record for geography type %s: %v\n", geography_type, err)
		}
	}

	fmt.Println("Completed Inserting Rows into the CCVI Details Table")
}
